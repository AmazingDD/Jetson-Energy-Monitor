...
sys.path.append("/home/orin/.local/lib/python3.10/site-packages")
# sys.path.append("/home/omnia/.local/lib/python3.8/site-packages/torchvision-0.13.0-py3.8-linux-aarch64.egg")
sys.path.append("/home/orin/.local/lib/python3.10/site-packages/torchvision-0.20.0-py3.10-linux-aarch64.egg")
...
def readValue(i2cAddr='0040', channel='0', valType='power',device='jetson_agx_orin'):
    ...
    if device == 'jetson_tx2':
        ...
        return res[valType]
    elif device == 'jetson_agx_orin': #TODO
        res = {}
        for valtype in ['voltage', 'current']:
            val = 'in' if valtype == 'voltage' else 'curr'
            ...
            with open(fname, 'r') as f:
                res[valtype] = f.read()
    elif device == 'jetson_orin_nx': #TODO
        res = {}
        for valtype in ['voltage', 'current']:
            val = 'in' if valtype == 'voltage' else 'curr'
            # Construct the filename for Jetson AGX Orin device
...
class PowerLogger:
    ...
    def showDataTraces(self, names=None,valType='power', showEvents=True, filename='CIL'):
    ...
    def showMostCommonPowerValue(self, nodeName='', valType='power', numBins=100, filename='CIL'):
        ...
        count, center = np.histogram(pwrData, bins=int(len(pwrData)*5)) #int(len(pwrData)*5))
        if self.figsave:
            import matplotlib.pyplot as plt
            ...
            plt.title('%s trace (NVPModel: %s)' % (valType, os.popen("nvpmodel -q | grep 'Power Mode'").read()[15:-1]))
            os.makedirs('results_test/power/%s' % (filename,) , exist_ok=True)
            plt.savefig('results_test/power/%s/%s_%s_histogram.png' % (filename, valType, nodeName))
            plt.close()
            print("showMostCommonPowerValue!")
        ...
        if self.csvwrite:
            import csv
            with open('results_test/power/%s/%s_histogram.csv' % (filename, valType),'w') as f:
                csvf = csv.writer(f)
...
if __name__ == "__main__":
    ...
    nodename = device_nodes[pl.device][2][0] # main 
